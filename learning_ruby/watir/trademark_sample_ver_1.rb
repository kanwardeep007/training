...
array = ['John Doe','Clothing, LLC','individual','HNO 2299 Sector8','faridabad','Alaska','Aruba',121006,9216617114,981411,'vinaymehta@gmail.com','www.helping.com'] 
...
browser.text_field(:name => 'form.owner[0].ownerName').wait_until_present.set order_last_name + order_first_name + order_middle_name----

#order_is_foreign means from outside US

      case order_company_type----
          when 'Corporation'
            browser.radio(:name => 'form.owner[0].entityType',:value => '03').wait_until_present.set
            unless order_is_foreign---- == true
                browser.select_list(:name => 'entityState').wait_until_present.select order_state----
              else
                browser.select_list(:name => 'entityCountry').wait_until_present.select order_outer_country----
            end
          when 'Individual'
              browser.radio(name: "form.owner[0].entityType", value: '01').when_present.set
            browser.select_list(name: "form.owner[0].citizenship").wait_until_present.select order_country----
          when 'Limited Liability Company'
            ...
            unless order_is_foreign---- == true
              browser.select_list(name: "entityState").select order_state----
            else
              browser.select_list(name: "entityCountry").select order_outer_country----
            end
          when 'Partnership'
            ...
            unless order_is_foreign---- == true
              browser.select_list(name: "entityState").select order_state
              browser.textarea(name: 'form.owner[0].partnerName').set order_partners_name_with_citizenship----
            else
              browser.select_list(name: "entityCountry").select order_outer_country----
            end
          ...
          when 'Joint Venture'
            ...
            unless order_is_foreign---- == true
              ...
              browser.select_list(name: "entityCountry").select s.submittable.client_entity_non_us_country+'X' unless s.submittable.client_entity_non_us_country == 'US'
            end
          when 'Sole Proprietorship'
              browser.radio(name: "form.owner[0].entityType", value: '19').when_present.set
            browser.select_list(name: "entityState").wait_until_present
            unless s.submittable.client_entity_is_foreign == true
              browser.select_list(name: "entityState").when_present.select_value s.submittable.client_entity_state
              browser.textarea(name: 'form.owner[0].partnerName').when_present.set s.submittable.client_general_partners
            else
              browser.select_list(name: "entityCountry").when_present.select_value s.submittable.client_entity_non_us_country+'X' unless s.submittable.client_entity_non_us_country == 'US'
            end
          when 'Trust'
              browser.radio(name: "form.owner[0].entityType", value: '17').when_present.set
            browser.select_list(name: "entityState").wait_until_present
            unless s.submittable.client_entity_is_foreign == true
              browser.select_list(name: "entityState").when_present.select_value s.submittable.client_entity_state
              browser.textarea(name: 'form.owner[0].partnerName').when_present.set s.submittable.client_general_partners
          ...
          when 'Estate'
              browser.radio(name: "form.owner[0].entityType", value: '18').when_present.set
            browser.select_list(name: "entityState").wait_until_present
            unless s.submittable.client_entity_is_foreign == true
              browser.select_list(name: "entityState").when_present.select_value s.submittable.client_entity_state
              browser.textarea(name: 'form.owner[0].partnerName').when_present.set s.submittable.client_general_partners
...
browser.text_field(name: 'fromName').when_present.set
